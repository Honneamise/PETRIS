...
{
    int32_t h = selector.h / selector.opt_count;

    PixieColorSet(gui.bg_color);
    PixieDrawRectFill(selector.x,selector.y,selector.w,selector.h);

    for(int32_t i=0;i<selector.opt_count;i++)
    {
    ...
    switch(event)
    {
        ...
        case P_MOUSEPRESS:
            if(selector->over)
            {
                ...
                *selector->selected = (y - selector->y) / ( selector->h / selector->opt_count); 
            }
            break;
...
/* SLIDER */
/**********/
static void PSliderDraw(PixieGui gui,PSlider slider)
{
    PixieColorSet(gui.bg_color);
...
/**********/
static void PSliderEvent(PixieGui *gui, PSlider *slider, int32_t id)
{
    int32_t event = PixieEventGetType();
...
/**********/
int32_t PixieGuiAddRadio(PixieGui *gui, int32_t x, int32_t y, int *status)
{
    PRadio radio = {0};

    radio.x = x;
    radio.y = y;
    radio.w = P_FONT_W * gui->font_scale;
    radio.h = P_FONT_H * gui->font_scale;
...
/**********/
int32_t PixieGuiAddToggle(PixieGui *gui, int32_t x, int32_t y, char *text, int *status)
{
    PToggle toggle = {0};
...
/**********/
int32_t PixieGuiAddSelector(PixieGui *gui, int32_t x, int32_t y, int32_t *selected, int32_t opt_count, char **options)
{
    ...
    selector.h = opt_count * PixieFontGetHeight();
...
int32_t PixieGuiAddSlider(PixieGui *gui, int32_t x, int32_t y, int32_t size, int32_t *val, int32_t min_val, int32_t max_val)
{
    ...
    slider.x = x;
    slider.y = y;
    slider.w = size * P_FONT_W * gui->font_scale;
    slider.h = P_FONT_H * gui->font_scale;
